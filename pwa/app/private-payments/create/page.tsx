...
export default function CreatePage() {
  ...
  const [autoGeneratedImage, setAutoGeneratedImage] = useState<File | null>(null);
  const [isUsingAutoImage, setIsUsingAutoImage] = useState(true);
  ...
  // Auto-generate card image when dialog opens
  useEffect(() => {
    if (link && cardRef.current) {
      const generateCardImage = async () => {
        try {
          ...
          canvas.toBlob((blob) => {
            if (blob) {
              const file = new File([blob], 'payment-request.png', { type: 'image/png' });
              setAutoGeneratedImage(file);
              if (isUsingAutoImage) {
      ...
      setTimeout(generateCardImage, 100);
    }
  }, [link]);
  ...
  const handleUseAutoImage = () => {
    if (autoGeneratedImage) {
      ...
      setIsUsingAutoImage(true);
      const reader = new FileReader();
      reader.onloadend = () => {
        setImagePreview(reader.result as string);
      };
      reader.readAsDataURL(autoGeneratedImage);
    }
  };
            ...
            >
              ...
              <div className="p-6 overflow-y-auto">
                <div className="space-y-4">
                  ...
                  <div ref={cardRef} className="relative rounded-xl bg-gradient-to-br from-gray-900 to-black dark:from-black dark:to-gray-950 border border-solana-purple/60 p-6 overflow-hidden shadow-lg shadow-solana-purple/20">
                  ...
                  <div className="bg-gray-50 dark:bg-gray-900 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-xs font-semibold text-gray-700 dark:text-gray-300">Share Image</p>
                      {imagePreview && (
                        <span className="text-xs px-2 py-1 rounded-full ${
                          ...
                          {isUsingAutoImage ? 'Auto-generated' : 'Custom'}
                        </span>
                      )}
                    </div>
                    <div className="flex gap-3">
                      {imagePreview ? (
                        <div className="relative">
                          <img src={imagePreview} alt="Preview" className="w-20 h-20 object-cover rounded-lg border-2 border-gray-300 dark:border-gray-600" />
                          {!isUsingAutoImage && (
                            <button
                              onClick={handleUseAutoImage}
                              className="absolute -top-2 -right-2 bg-neon-green text-black rounded-full w-5 h-5 flex items-center justify-center text-xs font-bold hover:bg-green-400 transition-colors"
                              title="Use auto-generated image"
                            >
                              ↻
                            </button>
                          )}
                      ...
                      <div className="flex-1 flex flex-col justify-center gap-2">
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          {isUsingAutoImage 
                            ? 'Using auto-generated payment card image' 
                            : 'Using your custom image'}
                        </p>
                        <label className="inline-flex items-center gap-1 text-xs text-purple-500 hover:text-purple-600 cursor-pointer font-medium">
                          <input
                            type="file"
                            accept="image/*"
